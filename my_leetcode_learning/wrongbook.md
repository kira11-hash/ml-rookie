...
1. 因为有序数组里，重复元素一定相邻
2. 所以不用新开 `new` 去全局查重
3. 只需要比较“当前元素”和“上一个保留元素”是否相等
4. 这就是双指针解法的核心

这条认识比单纯做出这道题更重要，会迁移到很多题。

---

### C. 你最初思路为什么不对（`new` 列表版）

你最初写法大意是：
1. 新建 `new = []`
2. 遍历 `nums`
3. 用 `if nums[i] in new` 判断是否重复
4. 尝试用 `nums[i] = nums[i+1]` 处理重复

主要问题：
1. `nums[i+1]` 在最后一位会越界
2. “改值”不等于“删除重复元素”
3. 没有维护题目要求的“前 k 个位置为去重结果”
4. `x in new` 查 list 是线性查找，复杂度会偏高
...
定义（统一心智模型版）：
1. `slow`：下一个写入位置（也可理解为当前有效长度）
2. `fast`：扫描指针，从左到右检查后续元素

流程：
1. 先让 `slow = 1`（第一个元素默认保留，所以当前有效长度是 1）
2. `fast` 从 `1` 开始遍历
3. 如果 `nums[fast] != nums[slow - 1]`
   - 说明遇到新元素
   - 把它写到 `nums[slow]`
   - `slow += 1`（有效长度 +1）
4. 最后返回 `slow`
...
为什么返回 `slow`：
1. 在这个写法里，`slow` 不是“最后一个下标”
2. `slow` 直接表示“当前有效长度 / 下一个写入位置”
...
1. 看见“按顺序排序的数组”，要本能想到：
   - 可以优先判断前后/相邻元素是否相等（利用有序性）

2. Python 写题不像 Verilog：
   - 不是每个 `if` 都必须写全 `else`
   - 有时候只需要写一个 `if` 来处理“满足条件时的动作”
   - 不满足条件时直接“什么都不做，继续循环”也完全合法

3. 对双指针有了新感悟：
   - `fast` 负责扫描
   - `slow` 负责维护“有效区间/写入位置”（你现在统一记成“下一个写入位置”非常好）
   - 这类题核心是先定义清楚两个指针的语义
...
1. 题目写了“有序数组”，一定要优先利用这个条件
2. 题目写了“原地修改”，优先想双指针，而不是新开数组
3. 这题不要求真的删短 `nums`，只要求前 `k` 个位置正确并返回 `k`

---

### H. 一句话记忆
“26 的题眼是 `有序 + 原地`：`slow` 记作下一个写入位置，`fast` 扫描新元素，遇到新值就写入并 `slow += 1`。”
...
给你一个**有序数组**，每个数**最多只能保留2个**，多余的删掉，原地修改，返回有效长度。

例如 `[1, 2, 2, 2, 3]` -> 有效部分变成 `[1, 2, 2, 3]`，返回 `4`。

---

## 核心思路，只有一句话

用 `slow` 维护一个**已整理好的有效区间** `nums[0:slow]`。

每来一个新元素 `nums[fast]`，只问一个问题：

> **这个数放进有效区间，会不会变成第3个重复？**

怎么判断？和有效区间**倒数第2个**比，也就是 `nums[slow-2]`：
- 相等 -> 说明已经有2个了，再放就是第3个 -> **跳过**
- 不相等 -> 没超限 -> **放入**
...
前两个元素**不管是什么，直接保留**。因为题目允许最多2个，前两个不可能违规。
...
def removeDuplicates(nums):
    ...
    return slow                   # slow就是有效区间的长度
```

---

## 举例：`[1, 2, 2, 2, 3]`（nums[0] != nums[1]）

**初始状态：** `slow = 2`，有效区间 = `[1, 2]`

---

**fast = 2，nums[fast] = 2**

比较 `nums[fast]` 和 `nums[slow-2]` = `nums[0]` = **1**

`2 != 1` -> 没超限，写入

```text
nums[slow] = nums[fast]  ->  nums[2] = 2
slow = 3
有效区间：[1, 2, 2]
```
...
比较 `nums[fast]` 和 `nums[slow-2]` = `nums[1]` = **2**

`2 == 2` -> 有效区间里已经有2个 `2` 了，再放就是第3个 -> 跳过
...
| 题目 | 要求 | 判断条件 |
|------|------|--------|
| 26题 | 每个数最多保留**1**个 | `nums[fast] != nums[slow-1]` |
| 80题 | 每个数最多保留**2**个 | `nums[fast] != nums[slow-2]` |

规律：最多保留k个，就和 `nums[slow-k]` 比。就这一个区别。
