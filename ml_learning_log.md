...
### 今日计划（承接 checklist）
- [x] 完成 Day6 checklist 全部项（含日志更新、手写复盘、wrongbook复习）
- [x] 复习 Day1-Day6 核心语法与训练流程
- [x] LeetCode 至少 1 题（实际完成超过 1 题）
- [x] 扩充 `my_leetcode_learning/wrongbook.md`
- [x] 为新一周（2.23起）做主线切换准备

### 时间记录
- 未单独精确计时（本次以 checklist 完成为主）
- 备注：后续建议恢复“开始/结束时间”记录，方便统计每周 5-10h 是否达标

### 今日完成（复习日实际产出）
- [x] Day6（MNIST baseline）结果复盘完成，关键报错已能口述
- [x] LeetCode `283. Move Zeroes` 完成（双指针，通过）
- [x] LeetCode `27. Remove Element` 完成（双指针，通过，理解 `del nums[slow:]` 是可选收尾）
- [x] LeetCode `219. Contains Duplicate II` 完成暴力法正确版，并理解哈希法标准解
- [x] 新建补充题模板：`219`、`27`（含中文题面）
- [x] 将 `1/217/167/283/27/219` 的思路讲解系统写入 `wrongbook.md`
- [x] `wrongbook` 按题号结构继续完善（错误、提问、复杂度、记忆句）

### 今日语法/代码复习重点（Day6 + LeetCode）
1. `for x in nums` 遍历的是元素值，不是下标；需要下标时用 `range(len(nums))` 或 `enumerate(nums)`
2. `x in mp`（字典）默认查的是 `key`，不是 `value`
3. `range(a, b)` 包含 `a`、不包含 `b`
4. Python 没有 `++`，写自增要用 `+= 1`
5. `fast += fast` 等价于 `fast = fast + fast`（会翻倍，不是前进一步）
6. `del nums[a:b]` 是切片删除；`del nums[slow:]` 常用于“本地收尾整理”
7. `.sum` 是方法对象，`.sum()` 才执行；`.sum().item()` 常用于从张量统计并取 Python 数值

### 今日报错与修复（复习日/刷题阶段）
1. 283 双指针初版错误：`for nums[fast] in nums`
   - 原因定位：`for` 左侧必须是变量，不可写索引表达式
   - 修复动作：改为 `for fast in range(len(nums))`
   - 复盘：双指针题本质是“下标控制”，先拿下标再访问数组

2. 283 指针更新错误：`fast += fast` / `slow += slow`
   - 原因定位：误把“前进一步”写成“加上自己”
   - 修复动作：统一改为 `+= 1`
   - 复盘：先写清每个指针语义，再写更新规则

3. 219 暴力法越界风险
   - 原因定位：`j = i + k` 后内层遍历可能访问到 `len(nums)` 之外
   - 修复动作：改为 `j = min(i + k, len(nums) - 1)`
   - 复盘：任何“窗口上界”都要先做边界裁剪

4. 219 哈希法初版遗漏首次写入
   - 原因定位：`mp[x] = i` 只在 `if x in mp:` 分支内更新，导致新值不入表
   - 修复动作：把 `mp[x] = i` 放到 `if` 外，每轮都执行
   - 复盘：219 核心是“每轮更新最近一次下标”

### 今日提问与答案（复习日，一一对应）
1. 问：复习 LeetCode 还需要再写一遍吗？  
   答：需要“选择性重写”——看题解才做出的题必须重写；不稳的题隔天再写；完全掌握可口述。

2. 问：Python 里如何交换两个变量？  
   答：可直接写 `a, b = b, a`（多重赋值）。

3. 问：Python 里如何删除 list 中元素？  
   答：常用 `remove`（按值删一次）、`pop`（按下标删并返回）、`del`（按下标/切片删）、`clear`（清空）。

4. 问：`for x in nums` 为什么不适合 283 的双指针交换？  
   答：因为它只有值没有下标；而 283/27 需要按下标原地交换或写入。

5. 问：`if need in mp` / `if x in mp` 在查什么？  
   答：查字典的 `key`；这也是 Two Sum/219 哈希解法的关键语义。

6. 问：219 先暴力再哈希这种思路常见吗？  
   答：非常常见；大多数人也是先写正确版，再做复杂度优化。

### 复习日总结（2.22）
- 本日定位正确：没有盲目上新，而是把 Day6 + 多道 LeetCode 的“模板意识”补起来。
- 最大收获：开始把题目归类成“哈希判重 / 双指针原地筛选”两大模板，而不是每题重开。
- 风险提醒：复习日容易变成“只看不写”；你这次通过实做 `283/27/219` 避免了这个问题。

### 自检（复习日）
- 我能区分 `for x in nums` 与 `for i in range(len(nums))` 的适用场景：`是`
- 我能解释 283 与 27 为什么是同类题：`是`
- 我能解释 219 为什么需要“最近一次下标”：`是`
- 我能口述 219 哈希法为什么要“每轮更新字典”：`是`
- 今日自评（1-10）：`6`

### 下周启动提醒（2026-02-23 起）
- 进入新阶段：`王树森课程 + 天池 ItemCF baseline + LeetCode 并行`
- 约束：每周 `5-10h`，单日 `<=2h`

---

## 2026-02-23（推荐系统课程启动日）

### 今日完成（已完成 5 节课）
- [x] 推荐系统基础：`概要01 推荐系统的基本概念`
- [x] 推荐系统基础：`概要02 推荐系统的链路`
- [x] 推荐系统基础：`概要03 推荐系统的AB测试`
- [x] 召回：`召回01 基于物品的协同过滤（ItemCF）`
- [x] 召回：`召回02 Swing模型`

### 今日学习内容（课程要点速记）
1. 推荐系统基本概念
   - 明确推荐系统不是“单一模型”，而是一个完整系统工程（召回/排序/重排 + 工程链路）。
   - 明确目标通常是业务指标（点击、时长、转化），不是只看模型精度。

2. 推荐系统链路
   - 建立了“召回 -> 排序 -> 重排”的主流程框架。
   - 理解不同阶段职责不同：召回重覆盖与速度，排序重精度，重排重业务规则/多样性等。

3. AB测试（实验、反转实验、推全）
   - 理解 AB 测试是线上对照实验，不是主观判断。
   - 理解“实验推全”是实验效果稳定后全量上线。
   - 理解“反转实验”是为了验证效果是否跟着方案走，而不是跟着分桶走。

4. ItemCF（基于物品协同过滤）
   - 理解核心思想：物品相似度来自用户共同行为（共同点击/共同喜欢）。
   - 理解 ItemCF 常见问题：热门物品偏置、重度用户噪声。

5. Swing
   - 理解 Swing 是在 ItemCF 基础上对“共同用户证据”做去噪加权。
   - 关键直觉：共同用户彼此越像，证据权重越小；共同用户越“跨圈层”，证据价值越高。

### 今日收尾任务（已完成）
- [x] `fun-rec 6.1` 赛题理解（已读完并完成主线理解）
- [x] 查看本地数据文件字段（`train/testA/articles`）
- [x] 用自己的话回答：输入 / 输出 / 评估指标

### 今日收尾完成内容（天池阶段一启动）
1. `fun-rec 6.1` 赛题理解完成
   - 明确任务本质：根据用户历史点击行为，预测用户最后一次点击文章（推荐问题，不是普通结构化分类题）。
   - 明确建模路线：先把业务问题转成可建模的监督学习/排序问题，再走“召回 -> 排序”。
   - 明确评估直觉：Top5 推荐中命中真实最后点击文章，且命中越靠前分越高（MRR 思想）。

2. 本地数据文件字段确认完成（已建立三表关系）
   - `train_click_log.csv` / `testA_click_log.csv`：用户点击行为日志（核心字段：`user_id`、`click_article_id`、`click_timestamp`）。
   - `articles.csv`：文章属性表（`article_id`、`category_id`、`created_at_ts`、`words_count`）。
   - 关联键：`click_article_id <-> article_id`。

3. 当前阶段的三问（三答）已能口述
   - 输入：用户点击日志 + 文章属性（后续可加 `articles_emb.csv`）。
   - 输出：每个测试用户的 Top5 推荐文章（按预测点击概率排序）。
   - 评估：命中位置倒数并对用户求平均（MRR 风格）。

### 今日补充问答（推荐系统主线启动）
1. 问：`embedding` 向量是什么意思？  
   答：把离散对象（词/用户/文章ID）映射成稠密数字向量，让模型能计算相似性；相似对象在向量空间里通常更接近。

2. 问：AB测试 / 实验推全 / 反转实验分别是什么？  
   答：AB测试是线上对照实验；推全是实验通过后的全量上线；反转实验是交换实验组与对照组以验证效果是否“跟着方案走”。

3. 问：哈希函数是什么、`hash` 这个词本身是什么意思？  
   答：哈希函数是把任意输入快速映射成固定形式值（常为整数）的规则；`hash` 原意接近“剁碎/杂烩”，计算机里引申为“快速映射编号/指纹”。

4. 问：算法里“正交”和“互斥”是什么意思？  
   答：互斥=不能同时成立/二选一；正交=可同时存在且相对独立、可分开优化（但不等于完全无关）。

5. 问：搜推里粗排/精排能叫“正交”吗？  
   答：模块职责上相对正交（可分层设计），但效果上有耦合（粗排决定精排看到的候选上限）。

6. 问：ItemCF 为什么会引出 Swing？  
   答：ItemCF 容易受热门物品和重度用户噪声影响；Swing 在共同用户基础上再按“用户对重合度”加权，降低噪声、提高相似度辨识度。

### 明日计划预告（2026-02-24，<=2h）
- 复看今日课程/赛题理解笔记（关键词级）
- 开始 `fun-rec 6.2 baseline`（先找入口，不抠细节）
- LeetCode 1 题（Python3，限时）

### 今日状态（阶段切换）
- 状态评价：课程框架已建立，符合“听完 ItemCF+Swing -> 开始天池阶段一”的触发条件。
- 执行策略：先做 `6.1/6.2` 跑通与读懂，不急着优化指标。

---

## 2026-02-24（天池 Baseline 跑通日，阶段性记录）

### 今日已完成（暂记）
- [x] 完成 `fun-rec 6.2 baseline` 代码落地（将网页代码整理为本地可运行脚本）
- [x] 修正 baseline 脚本中的路径问题（改为 `news_rec/data_raw` 与 `temp_results`）
- [x] 清理网页复制导致的说明文字/无效内容（如 `Copy to clipboard` 等）
- [x] 本地成功跑通 ItemCF baseline（小样本调试版）
- [x] 成功生成提交文件 `itemcf_baseline_02-24.csv`

### Baseline 跑通结果（关键输出）
- `all_click_df shape = (20000, 9)`：小样本点击日志总表（约 2 万条点击记录，9 列字段）
- `recall_df shape = (77210, 3)`：召回结果表（约 7721 个用户 × 每人 10 条召回）
- `tst_recall shape = (41020, 3)`：测试用户召回结果（约 4102 个测试用户 × 每人 10 条召回）
- 产物文件：
  - `temp_results/itemcf_i2i_sim.pkl`（物品相似度矩阵缓存）
  - `temp_results/itemcf_baseline_02-24.csv`（提交文件）

### 今日理解进展（很重要）
1. baseline 的核心不是“精排模型”，而是：
   - `ItemCF 召回 + 简单分数排序 + 生成提交文件`
2. 当前看到的 `shape` 数值属于“中间统计/流程规模检查”，不是效果指标
3. 当前脚本没有线下 `MRR` 评估，只是生成 `testA` 提交文件
4. `pipeline` 在这里指“完整处理流程链”（读数据 -> 相似度 -> 召回 -> 提交），不是 IC 微架构流水线

### 下一步（你已确认）
- 主线：看懂 baseline 代码骨架（先懂 `main + 关键函数输入输出`）
- 并行：LeetCode 1 题（保持节奏）

### 待你完成后再补（今晚/明天）
- [ ] LeetCode 题目记录（题号、思路、是否独立做出）
- [ ] `wrongbook.md` 更新
- [ ] 今日自评（1-10）
